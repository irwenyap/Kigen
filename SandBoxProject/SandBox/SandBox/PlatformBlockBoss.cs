...
namespace SandBox
{
    public class PlatformBlockBoss : UnchargedBlock
    {
        ...
        public bool freezeUCB = false;

        //Camera
        private CameraScript camera;
        private PlatformScriptedMovementController cutsceneController;
        ...
        protected override void OnInit()
        {
            ...
            cutsceneController = FindEntityByName("ScriptedMovement_Platform Cutscene")?.As<PlatformScriptedMovementController>();
        }
        protected override void OnUpdate(float dt)
        {
            if (scene1) return;

            if(transform.Translation.y >= bottomLimit)
            {
                if (freezeUCB) return;
                else
                {
                    camera.CameraShake(0.35f);
                    Audio.PlaySound(this.ID,"../Assets/Audio/Block SFX/PLATFORM STOP.wav", 0.8f);
                    freezeUCB = true;
        ...
        private void LoopAudio()
        {
            if (!freezeUCB)
            {
                playing = true;
                timer = 0f;
                Audio.PlaySound(this.ID, "../Assets/Audio/Block SFX/Platform Loop.wav", 0.3f);
            }
        }
        protected override void OnCollisionEnter(Collision collision)
        {
            if (collision.collider != null)
            {
                if (collision.collider.Entity.ID == player.ID)
                {
                    ...
                    if (!freezeUCB)
                    {
                        camera.followSpeed = 1f;
                        cutsceneController?.StartCutscene();
                        //Audio.PlaySound(this.ID,"../Assets/Audio/Voiceovers/Dialogue99-104.wav", 1f);
                    }
                }
            }
        }
        ...
        protected override void OnCollisionStay(Collision collision)
        {
            if(collision.collider != null)
            {
                if(collision.collider.Entity.ID == player.ID && freezeUCB)
                {
                    Activate(false);
                    movePlayer = false;
                }
